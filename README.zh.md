...
iriz 住在菜单栏里。输入法切错了，它把那句话改回来。你按键说话再按一次，它把话写成文字。它还能把零散的口述变成给智能体的任务，也能顺手翻译。

![主窗口：口述历史与设置](docs/assets/screens/page-history.png)

接下来才是它反复打磨的部分。文字落进输入框后你改了一个词，iriz 会注意到，并问要不要记住这个替换。口语里的停顿和重复在你自己的机器上清理干净。会议或庭审的录音按发言人分开，名字由你手动填上，记录就放在录音旁边。

![会议：把录音拖进来，记录就在旁边](docs/assets/screens/page-meetings.png)

语音在这台 Mac 上、在 Neural Engine 上识别。
...
按键，说一句话，再按一次。文字落在光标刚才闪动的地方：邮件、聊天窗口、终端。

要是没落进去，那条小胶囊会展开成面板，你从那里把文字取走。

识别语言直接在胶囊上右键切换。人总是在开口前一秒才决定说哪种语言，为这个跑去设置没有道理。
