...
[English](ONBOARDING.md) · [简体中文](ONBOARDING.zh.md) · [README](../README.ru.md)

С чего начать: пройди по порядку [установку](#установка), [первый запуск](#первый-запуск) и [проверку](#проверка).

Статус: инструкция готова для Маков с Apple Silicon на macOS 14 или новее; Intel не поддерживается.
...
Вот значок, который появится у вас в строке меню. В обычной диктовке сказанное превращается в текст прямо на вашей машине, а аудио никуда не уходит. Отдельные агентские режимы могут отправить уже распознанный текст только после вашего согласия.
...
Нужен Мак с Apple Silicon на macOS 14 или новее, с местом для Parakeet примерно на 500 МБ и временных файлов загрузки. Это модель для установки одной кнопкой. Этот файл не поддерживает Intel. Для сборки из исходников используется проверенный Xcode 26.6 (macOS 26 SDK, Swift 6.3.3); готовому DMG Xcode и терминал не нужны.
...
4. **Либо соберите сами, вместо шагов 2 и 3.** В терминале:
   ...
   Первая сборка тянет зависимости, поэтому следующие будут короче. В конце готовое приложение окажется в `/Applications`, а значок появится в строке меню. Исходники можно также скачать как [ZIP](https://github.com/zarubinvibe/iriz/archive/refs/heads/main.zip), распаковать и запустить ту же команду внутри папки. Установщику все равно нужен git.

5. **Откройте приложение первый раз.** Выпуск подписан ad hoc, без сертификата Developer ID и нотаризации Apple. macOS не подтверждает издателя.
...
7. **Скачайте модель самым коротким способом.** Экран **«Установи распознавание речи»**, кнопка **«Скачать и установить Parakeet»**. Приложение скачает, установит и выберет Parakeet для распознавания.
   ...
   Пропустили модель или хотите повторить загрузку позже? Откройте **iriz → Настройки → Диктовка → Скачать модель Parakeet…**. Если модель уже установлена, кнопка называется **«Настроить распознавание…»**. Автоматическая установка есть только для Parakeet; он не распознает китайскую речь. Автор вместо него использует установленную вручную Whisper large-v3-turbo. [Сравните поддерживаемые профили и кандидатов для скачивания](MODELS.ru.md). Язык интерфейса и язык речи настраиваются отдельно.
...
Тринадцать страниц: история, язык, расшифровка файлов, встречи, клавиши, раскладка, диктовка, плашка, словарь замен, заготовки, промпт-режим, перенос, место на диске.
