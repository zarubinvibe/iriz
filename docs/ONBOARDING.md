...
You need an Apple Silicon Mac on macOS 14 or newer, with space for a roughly 500 MB speech model and its temporary download files. This download does not support Intel. Building from source uses the tested Xcode 26.6 toolchain (macOS 26 SDK, Swift 6.3.3); the ready DMG needs no Xcode or Terminal.

For the current download, first launch, model recovery and manual updates, follow the [installation guide](DOWNLOAD.md). The walkthrough below explains the app's other screens.
...
1. **Get the app. Two doors, either one works.** The short door is the ready disk image from [Releases](https://github.com/zarubinvibe/iriz/releases): download the `.dmg`, open it, drag the icon onto `Applications`. Inside that image sit two objects and an arrow between them, nothing else.
   ...
   You can also download the source [ZIP](https://github.com/zarubinvibe/iriz/archive/refs/heads/main.zip), unpack it and run the same command inside the folder. The installer still requires Git. Doing any of this for the first time? Open the project in Claude Code and run `/iriz-setup`: the install goes as a conversation, one question at a time.
...
3. **Open a downloaded build for the first time.** This release is self-signed, not notarized by Apple. macOS cannot verify its publisher.

   If macOS says the publisher cannot be verified or Apple cannot check the app, proceed only if you trust this repository and have checked the download. Try opening iriz from Applications, then use System Settings → Privacy & Security → Open Anyway. See [Apple's explanation](https://support.apple.com/en-us/102445).

   If macOS says the app is damaged, stop and download it again from the official release; do not assume the file is intact. Do not bypass a malware warning or disable Gatekeeper. If the warning remains, stop and report it.
...
5. **Start the model download early.** On the **Install speech recognition** screen, press **Download and install Parakeet**. The app downloads Parakeet TDT v3, installs it and selects it for recognition. The model is not included in the DMG. Parakeet is the only model installed automatically; it does not recognize Chinese speech. Interface language and speech language are separate.

   The screen shows progress for the roughly 500 MB download and offers **Try again** after an error. You can continue to permissions while it downloads, but wait for **Model is in place** before trying dictation. Download time depends on the connection.

   Skipped the model or need to retry later? Open **iriz → Settings → Dictation → Download Parakeet model…**. If a model is already installed, the button reads **Set up speech recognition…**. Other engines require separate model installation, outside this guide.

   The next screen explains where iriz lives. The icon is at the top right of the screen, near the clock. No dock icon, no main window. History, dictionary and settings all open from that icon. Continue from there to permissions.

6. **Microphone.** Press the button on this step and answer the system prompt.

   You see the step switch to granted. Without microphone access, iriz cannot hear your voice for dictation.
...
9. **Say your first sentence.** Once the model is ready, the next step shows your dictation key and a field to test it in. Press the key, say something out loud, press it again. "Hello, this is a test, can you hear me" is plenty.
...
For a DMG installation, follow [manual updating and rollback](DOWNLOAD.md#update-or-go-back): quit iriz before replacing it, and keep the previous app or image until the new one works.

If you build from source, open the project in Claude Code and run `/iriz-update`. It shows what changed before touching a single file, pulls only fast-forward changes, and re-checks itself afterwards. Your settings, dictionary and history sit outside the project folder, so an update does not reach them.
