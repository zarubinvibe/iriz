...
| Что | Источник | Лицензия | Текст |
|---|---|---|---|
| переключение раскладки | `rashn/RuSwitcher` | MIT | `THIRD-PARTY/RuSwitcher-LICENSE` |
| диктовка | `shlgd/SuperDictate` | MIT | `THIRD-PARTY/SuperDictate-LICENSE` |
| распознавание речи | `FluidInference/FluidAudio` | Apache-2.0 | `THIRD-PARTY/FluidAudio-LICENSE` |
| модель речи | `FluidInference/parakeet-tdt-0.6b-v3-coreml` | CC-BY-4.0 | `THIRD-PARTY/parakeet-CC-BY-4.0` |
...
- `Sources/SmltlkCore/` (5): Dict.swift · KeyCodes.swift · KeyMapping.swift ·
  LayoutDetector.swift · ShortWords.swift
- `Sources/SmltlkInput/` (7): AutoSwitchPolicy.swift · DynamicKeyMapping.swift ·
  KeyboardMonitor.swift · LayoutSwitcher.swift · SettingsManager.swift ·
  SpotlightAX.swift · TextConverter.swift
- `Sources/SmltlkApp/` (1): AppDelegate.swift — несёт двойную отметку, RuSwitcher
  и SuperDictate
...
- 03.08.2026 — пакет разложен на таргеты Core / Input / App (коммит `dbc8ce9`).
  ...
  в `SmltlkCore`; `DynamicKeyMapping.swift`, `KeyboardMonitor.swift`, `LayoutSwitcher.swift`,
  `SettingsManager.swift`, `SpotlightAX.swift`, `TextConverter.swift` — в `SmltlkInput`.
- 04.08.2026 — `main.swift` донора удалён (коммит `113fa91`): оболочка переписана
  на `MenuBarExtra`, точка входа теперь `Sources/SmltlkApp/SmltlkAppMain.swift` — свой код,
...
- `Sources/SmltlkDictate/` (17): AudioCapture.swift · DictationHistoryWindow.swift ·
...
- `Sources/SmltlkSettings/` (1): HotkeyRecorderController.swift
- `Sources/SmltlkApp/` (1): AppDelegate.swift — двойная отметка, RuSwitcher и SuperDictate

`Sources/SmltlkImport/SuperDictateImporter.swift` в этот список не входит: донорского кода
...
`Sources/SmltlkSettings/HotkeyRecorderController.swift`, 212 строк. Панель записи горячей
