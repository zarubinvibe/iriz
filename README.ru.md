...
Результат: говоришь вслух, на экране появляется текст. Речь разбирает твой Мак. Передачу текста агенту ты разрешаешь отдельно.
...
<p align="center">Настройка: Быстрый первый запуск: установи Parakeet, около 500 МБ. Автор использует Whisper large-v3-turbo; перед выбором открой инструкцию по моделям.</p>

<p align="center">Безопасность: Подпись ad hoc, без сертификата Developer ID и нотаризации Apple. Этот выпуск не поддерживает Intel.</p>
...
**Почему «iriz».** Ирида у греков была вестницей богов и радугой, мостом между небом и землей. Она не сочиняла послание, а доносила его целым. Отсюда задача программы: переносить речь в текст. Распознавание может ошибаться, поэтому результат нужно проверить.
...
Наведи на нее мышь, и капля раскроется в ряд кнопок: запись, промпт, перевод, язык, история, настройки. Язык переключается здесь же, за секунду до того, как заговорить.
...
Распознавание идет локально на твоем Маке. Скорость и используемое железо зависят от модели: Parakeet работает быстро и ставится автоматически, а моделям Whisper нужен файл, установленный вручную. Аудио остается на машине. Дальнейшая обработка текста агентом требует твоего согласия.
...
Для самого короткого первого запуска нажми «Скачать и установить Parakeet» в знакомстве. Это единственная модель, которую iriz ставит автоматически; загрузка занимает около 500 МБ. Автор вместо нее использует установленную вручную Whisper large-v3-turbo на MacBook Air M3 с 24 ГБ памяти. [Выбери модель распознавания](docs/MODELS.ru.md), затем выдай разрешения и попробуй клавишу диктовки.
...
| Neural Engine | Часть Apple Silicon, которую используют некоторые местные модели. Другие могут работать через Metal или процессор |
...
- Экран не читается: ни ScreenCaptureKit, ни снимков окон. Чтобы предложить исправления после диктовки, iriz через Accessibility читает только поле в фокусе и не сохраняет его полное содержимое.
...
- Приложение подписано ad hoc, без сертификата Developer ID и нотаризации Apple. Предупреждение о первом запуске есть в инструкции скачивания.
- Сейчас iriz поддерживает три профиля распознавания. Автоматически ставится только Parakeet; файлы Whisper устанавливаются вручную. Языки, место на диске и скорость отличаются.
- Обновление ручное: скачай новый DMG, закрой iriz и замени приложение. Модели и история хранятся вне его пакета.

Дальше: [выбрать модель распознавания](docs/MODELS.ru.md), [открыть пошаговое знакомство со скриншотами интерфейса](docs/ONBOARDING.ru.md), [прочитать историю выпусков](CHANGELOG.ru.md), [помочь проекту](CONTRIBUTING.ru.md), [прочитать о безопасности](SECURITY.ru.md).
...
Публичные [проекты семьи Olympuz](https://github.com/zarubinvibe/athena#olympuz-family) собраны ниже. Из таблицы можно открыть репозиторий или сразу скачать исходники в ZIP.
...
Дальше: [скачай DMG для macOS](https://github.com/zarubinvibe/iriz/releases/latest/download/iriz-macos-arm64.dmg) и попробуй первую диктовку.
