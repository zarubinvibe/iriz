...
Говори вслух, на экране появляется текст. Разбирает речь сам Мак, ничего не улетает в чужое облако.
...
Умеет и другое. Чинит фразу, набранную не в той раскладке. Поправишь слово после вставки, и программа предложит запомнить замену. Уберет из речи запинки и повторы. Разберет запись встречи или судебного заседания по голосам, а рядом со звуком положит готовый протокол.
...
**Главное преимущество:** речь разбирает твой Мак, и во время диктовки сеть закрыта рубильником в коде, а не обещанием в тексте.
...
Рядом с курсором появляется небольшая плашка, ее волна идет за твоим голосом, и видно, что тебя слышат, а не приходится гадать. Звук живет в памяти ровно столько, сколько нужно на разбор, и на диск не попадает.
...
Распознавание идет на Neural Engine твоего Мака: семь секунд речи разбираются примерно за десятую долю секунды. Никуда ничего не отправляется, а пока идет диктовка, загрузчик внутри библиотеки распознавания закрыт рубильником.
...
**Что получится:** расшифровка, которая нигде, кроме твоей машины, не была.
...
[Установка, восстановление модели и обновления](docs/DOWNLOAD.ru.md). Для сборки из исходников нужен Xcode 26 или новее с macOS 26 SDK и Swift 6. Ниже команды и другие способы установки.
...
|---|---|---|---|---|---|
| **iriz** | Диктуешь рабочее и отдать не можешь | Раскладка, местная диктовка, речь в задание | Никуда, разбирает Мак | Да, английский и русский | Русский первым, нотаризации пока нет |
...
- Расшифровки лежат под правами 0700 и 0600 и остаются на твоем диске.

В сеть уходит одно и по твоей кнопке: разовая загрузка модели распознавания. Промпт-режим выключен по умолчанию, и включая его, ты соглашаешься, что расшифровка уйдет выбранному агенту.
