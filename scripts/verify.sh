...
echo
echo "=== ворота целостности плашки ==="
# Форма обязана помещаться в своё окно целиком. Стекло оставалось формой
# закрытой панели, окно резало его по кромке, и владелец видел «сломанную
# плашку» - на кадре витрины и на своём экране. Глазами это ловилось раз в
# несколько дней, замером ловится каждым прогоном.
bash scripts/plate_whole_gate.sh || FAIL=1
