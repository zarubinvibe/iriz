...
NACHALO="$(date +%s)"
# </dev/null обязателен. Без него codex, запущенный без терминала (фоном, из
# скрипта, из агента), печатает «Reading additional input from stdin...» и ждет
# вечно: кадр не снимается, файл не появляется, а обертка молчит до таймаута.
# Ловилось живьем 05.09.2026 на двух кадрах подряд.
codex exec \
  --cd "$DOM" \
  --sandbox danger-full-access \
  --skip-git-repo-check \
  "$ZADANIE" >"${VYHOD_ABS}.log" 2>&1 </dev/null || true
KONETS="$(date +%s)"
