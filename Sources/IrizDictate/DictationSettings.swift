...
public final class DictationSettings: @unchecked Sendable {
    ...
    /// Личный выбор владельца - Whisper large-v3-turbo с подсказкой декодеру.
    /// Свежая установка сначала предлагает Parakeet: только для него есть
    /// автоматический установщик.
    ///
    /// Замер 03.09.2026 сравнивал Parakeet с полной Whisper large-v3,
    /// а не с Turbo. Полная large-v3 лучше распознала смешанную русскую
    /// и английскую техническую речь. Это доказывает выбор семейства
    /// Whisper, но равноценных цифр WER и скорости для Turbo в репозитории нет.
    ///
    /// Turbo выбрана как меньшая модель того же семейства: её `.bin` занимает 1,5 ГиБ
    /// против 2,9 ГиБ у полной large-v3. Прежний движок остается в том же бинарнике
    /// и выбирается здесь же.
