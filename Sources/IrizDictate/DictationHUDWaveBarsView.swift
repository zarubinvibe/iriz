...
/// Кайма окна вокруг стекла - место, куда светит контур.
///
/// Свет рисуется НАРУЖУ от кромки стекла, и без каймы его срезало окном: на
/// кадре записи горел один левый бок. Число - самый широкий слой ореола плюс
/// запас на размытие.
let DICTATION_HUD_GLOW_PAD: CGFloat = 10

let DICTATION_HUD_MODE_GLOW_STRENGTH: CGFloat = 0.55

/// Сколько света отдаёт контур на этой стадии.
///
/// Покой светит ТИШЕ работы, но светит: он висит на экране весь день, и
/// гореть в полную силу ему не за что - а не гореть вовсе значит пропасть на
/// светлом столе. Обрыв ярче всех: молчать про потерю нельзя.
func dictationHUDContourGlowStrength(stage: DictationHUDStage,
                                            tone: DictationHUDWaveTone) -> CGFloat {
    switch stage {
    case .resting: return 0.42
    default: return tone == .failure ? 0.85 : DICTATION_HUD_MODE_GLOW_STRENGTH
    }
}
...
func dictationHUDWaveTone(stage: DictationHUDStage,
    ...
    case .notDelivered, .promptNotDelivered, .promptFailed,
        ...
        return .failure
    case .buildingPrompt:
        // Режим держится ДО КОНЦА работы, а не до конца записи. Перевод идёт
        // через того же агента, что и промпт, и попадал сюда же - лента
        // начинала фиолетовой и посреди работы становилась золотой. Владелец
        // 07.09.2026: «вначале говорит фиолетовым, а потом золотым. Должно быть
        // фиолетовый и распознавание тоже фиолетовое, золотой - распознавание
        // тоже золотое».
        return purpose == .translation ? .translation : .prompt
    case .resting:
