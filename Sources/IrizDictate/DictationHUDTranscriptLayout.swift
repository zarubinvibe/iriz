...
/// Сколько длится сжатие плашки в покой и обратно.
///
/// То же число, что и у раскрытия в панель, и это НЕ совпадение: покой,
/// запись и панель - три вида одного окна, и разная длительность читалась бы
/// как разные механизмы. Своё имя у константы затем, что менять её можно
/// отдельно: сжатие в покой владелец видит десятки раз в день, раскрытие
/// панели - редко.
public let DICTATION_HUD_REST_MORPH_SECONDS: TimeInterval = DICTATION_HUD_TRANSCRIPT_MORPH_SECONDS
